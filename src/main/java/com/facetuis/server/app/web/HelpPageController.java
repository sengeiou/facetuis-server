...
public class HelpPageController extends FacetuisController {
    ...
    public BaseResponse list(){
        ...
        hp5.setContent("自买的商品在“我的订单”里面看，团队的订单在我的团队里面看");
        HelpPage hp6 = new HelpPage();
        hp6.setTitle("自己够买的，为什么在拼多多上面看不到订单？");
        hp6.setContent("请注意查看订单出单时间，并非消息发送时间，所以请悉知");
        ...
        HelpPage hp14 = new HelpPage();
        hp14.setTitle("下载App后，输入手机号及验证码就自动登陆了，没输入邀请码是为什么呢？");
        hp14.setContent("之前在微信授权注册过，在登陆App时，如果选择了手机号登陆方式，只需要输入验证码即可。");
        HelpPage hp15 = new HelpPage();
        hp15.setTitle("在微信里面注册过之后，可以直接在App里面选择微信登陆吗？");
        hp15.setContent("可以的");
        HelpPage hp16 = new HelpPage();
        hp16.setTitle("拼多多客服电话有吗？");
        hp16.setContent("之前在微信授权注册过，在登陆App时，如果选择了手机号登陆方式，只需要输入验证码即可。");
        HelpPage hp17 = new HelpPage();
        hp17.setTitle("消费者热线：021-61897088");
        hp17.setContent("点击别人发发的邀请链接后，提示微信已经绑定，怎么办？");
        HelpPage hp18 = new HelpPage();
        hp18.setTitle("说明您的微信已经授权过，不需要再次注册，可以直接下载App登陆使用");
        ...
        hp21.setTitle("授权登陆的微信号要先设置好微信号，否则可能会授权不了，请先打开手机微信，设置好微信号，重新打开脸推App授权即可。");
        hp21.setContent("登陆App时，点绑定微信后无法正常登陆？");
        ...
        hp27.setTitle("下载App后，输入手机号及验证码就自动登陆了，没输入邀请码是为什么呢？");
        hp27.setContent("为什么我在APP里面自购的时候会需要拼团呢？");
        HelpPage hp28 = new HelpPage();
        hp28.setTitle("这种情况一般是您手机安装了拼多多APP导致，需要等拼多多App升级后，可以解决该问题，您也可以换个手机下单，就可以解决。");
        hp28.setContent("之前在微信授权注册过，在登陆App时，如果选择了手机号登陆方式，只需要输入验证码即可。");
